...
public final class SiteManager {
    ...
    private SiteManager(Context context) {
        ...
        if (clean(store.getString(context,SHARED_PREFERENCES_SITES_KEY, EMPTY_STRING)).isEmpty()) {
    ...
    public boolean exists(Context context,String site) {
        return Arrays.asList(getSites(context)).contains(clean(site));
    }
...
*/
    ...
    public String[] addSite(Context context, String site) {
        ...
        if (!Arrays.asList(getSites(context)).contains(clean(site)))
    ...
    public void setLastVisitedSite(Context context, String site){
        if (siteIsValid(site)) store.setString(  context,SHARED_PREFERENCES_LAST_VISITED_SITE_KEY, clean(site));
    }
    ...
    public void setLastSearchTerm(Context context, String term){
        store.setString(  context,SHARED_PREFERENCES_LAST_SEARCH_TERM_KEY, clean(term));
    }
    ...
    private boolean siteIsValid(String site){
        return clean(site).startsWith("http");
    }
}
